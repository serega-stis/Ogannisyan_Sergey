...
data = csv_filter(*csv_reader('vacancies_dif_currencies.csv'))
total = []

for current_vacancy in data:
    ...
    total.append({'name': current_vacancy.name, 'salary': salary, 'area_name': current_vacancy.area_name,
                   'published_at': str(current_vacancy.published_at)})
    if len(total) == 100:
        break
        
pd.DataFrame.from_records(total).to_csv('result.csv')
